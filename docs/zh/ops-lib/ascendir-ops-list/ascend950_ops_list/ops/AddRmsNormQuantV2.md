...
REG_OP(AddRmsNormQuantV2)
    ...
    .OUTPUT(resOut, TensorType({DT_FLOAT16, DT_FLOAT, DT_BF16}))
    .ATTR(axis, Int, -1)
    .ATTR(epsilon, Float, 1e-6f)
    .ATTR(div_mode, Bool, true)
    .ATTR(dst_type, Int, DT_INT8)
    .ATTR(output_res, Bool, false)
    .OP_END_FACTORY_REG(AddRmsNormQuantV2)
```

## Brief

AddRmsNormQuantV2 operator interface implementation.
...
- scales1: A tensor. Describing the weight of the first quant operation.
             Support dtype: float32/float16/bfloat16, support format: ND.
- scales2: An optional input tensor. Describing the weight of the second quant operation.
             Support dtype: float32/float16/bfloat16, support format: ND.
...
- zero_points2: An optional input tensor. Describing the bias of the second quant operation.
                  Support dtype: int32/float32/float16/bfloat16, support format: ND.
- bias: An optional input tensor. Describing the bias of the rmsnorm operation.
                  Support dtype: float32/float16/bfloat16, support format: ND.
...
- x: A tensor. Describing the output of the x1+x2 add operation.
                 Support dtype: float32/float16/bfloat16, support format: ND.
- resOut: An optional tensor. Describing the output of the rmsnorm operation before bias addition.
                       Support dtype: float32/float16/bfloat16, support format: ND.
...
- div_mode: An optional attribute. When div_mode is true, the quant operation uses division, otherwise, uses
multiplication.
...
- output_res: An optional bool. Whether to output resOut (rmsnorm result before bias).
                Defaults to false. When true, resOut output is computed and returned.
...
- input3 scales1: bfloat16,float16,float32
- input4 scales2: bfloat16,float16,float32
- input5 zero_points1: bfloat16,float16,float32,int32
- input6 zero_points2: bfloat16,float16,float32,int32
- input7 bias: bfloat16,float16,float32
- output0 y1: float8_e4m3fn,float8_e5m2,hifloat8,int8
- output1 y2: float8_e4m3fn,float8_e5m2,hifloat8,int8
- output2 x: bfloat16,float16,float32
- output3 resOut: bfloat16,float16,float32
