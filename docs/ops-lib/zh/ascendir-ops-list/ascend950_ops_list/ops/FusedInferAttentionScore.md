...
Function FusedInferAttentionScore.

## Inputs

- query: A matrix tensor. The type support int8, float16, bfloat16, hifloat8, float8_e5m2, float8_e4m3fn.
Query for attention structure.
- key: It's a dynamic input. A matrix tensor. The type support int8, float16, bfloat16, hifloat8, float8_e5m2, float8_e4m3fn, int4, float4_e1m2, float4_e2m1.
Key for attention structure.
- value: It's a dynamic input. A matrix tensor. The type support int8, float16, bfloat16, hifloat8, float8_e5m2, float8_e4m3fn, int4, float4_e1m2, float4_e2m1.
Value for attention structure.
- pse_shift: A matrix tensor. The type support float16, bfloat16.
Postional encoding parameters inside the attention structure.
- atten_mask: A matrix tensor. The type support float16, bool, uint8, int8.
Mask of the result QK, indicating whether to calculate the correlation between tokens.
- actual_seq_lengths: A matrix tensor. The type support int64.
Efective sequence length of query in different batches.
- actual_seq_lengths_kv: A matrix tensor. The type support int64.
Effective sequence length of key/value in different batches.
- dequant_scale1: A matrix tensor. The type support uint64, float32.
Dequantization factor behind BMM1.
- quant_scale1: A matrix tensor. The type support float32.
Quantification factor in front of BMM2.
- dequant_scale2: A matrix tensor. The type support uint64, float32.
Dequantization factor behind BMM2
- quant_scale2: A matrix tensor. The type support float32, bfloat16.
Quantization factor of the output.
- quant_offset2: A matrix tensor. The type support float32, bfloat16.
Qantization offset of the output.
- antiquant_scale: A matrix tensor. The type support float16, bfloat16.
Antiquantization factor.
- antiquant_offset: A matrix tensor. The type support float16, bfloat16.
...
- query_padding_size: A matrix tensor. The type support int64.
Whether the data in each batch of query is right-aligned, and the number of right-aligned entries.
- kv_padding_size: A matrix tensor. The type support int64.
Whether the data in each batch of key/value is right-aligned, and the number of right-aligned entries.
- key_antiquant_scale: A matrix tensor. The type support float16, bfloat16, float32.
The dequantization factor of key when separating the KV antiquantization parameters.
- key_antiquant_offset: A matrix tensor. The type support float16, bfloat16, float32.
The dequantization offset of key when separating the KV antiquantization parameters.
- value_antiquant_scale: A matrix tensor. The type support float16, bfloat16, float32.
The dequantization factor of value when separating the KV antiquantization parameters.
- value_antiquant_offset: A matrix tensor. The type support float16, bfloat16, float32.
The dequantization offset of value when separating the KV antiquantization parameters.
- key_shared_prefix: A matrix tensor. The type support int8, float16, bfloat16.
The input of the system prefix part of key in the attention structure.
- value_shared_prefix: A matrix tensor. The type support int8, float16, bfloat16.
The input of the system prefix part of value in the attention structure.
- actual_shared_prefix_len: A matrix tensor. The type support int64.
Effective Sequence Length of key_shared_prefix/value_shared_prefix.
- query_rope: A matrix tensor. The type support int8, float16, bfloat16.
The rope information of query in the MLA structure.
- key_rope: A matrix tensor. The type support int8, float16, bfloat16.
The rope information of key in the MLA structure.
- key_rope_antiquant_scale: A matrix tensor. The type support float16, bfloat16.
The dequantization factor of the rope information of key in the MLA structure.
- dequant_scale_query: A matrix tensor. The type support float32.
The dequantization factor of query.
- learnable_sink: A matrix tensor. The type support bfloat16.
The sink token factor of attention score.
- q_start_idx: A matrix tensor. The type support int64.
start idx of q for alibi pse.
- kv_start_idx: A matrix tensor. The type support int64.
start idx of kv for alibi pse.

## Outputs

- attention_out: A matrix tensor. The type support float16, int8, bfloat16.
...
The result of query multiplied by key minus softmax_max is then exponentiated, followed by summing to obtain softmax_sum.
Finally, take the log of softmax_sum and add the result obtained from softmax_max.

## Attributes

- num_heads: An int. The number of the heads.
- scale: A float. The scale value. Default: 1.0.
- pre_tokens: An int. Previous tokens. Default: 2147483647.
- next_tokens: An int. Next tokens. Default: 2147483647.
...
- 4: band mask
- inner_precise: An int. 0, float16 high precision. 1, high performance. 2, high precision with row invalid fix. 3, high performance with row invalid fix. Default: 1.
- block_size: An int. Default: 0.
In PageAttention, KV stores the maximum number of tokens in each block.
- antiquant_mode: An int. Antiquantization mode, 0: per-channel (per-channel includes per-tensor); 1: per-token. Default: 0.
- softmax_lse_flag: A bool. Whether to output softmax_lse. Default: false.
- key_antiquant_mode: An int. Antiquantization mode of key. Default: 0.
...
- 6: per-token-group
- value_antiquant_mode: An int. Antiquantization mode of key. The mode number is the same as key_antiquant_mode. Default: 0.
- query_quant_mode: An int. Quantization mode of key. Only support mode 3: per-token+per-head now. Default: 0.
- pse_type: An int. Type of pse, 0: generate pse outside, 2: generate pse inside, 3: generate pse inside and do sqrt. Default: 0.
- out_dtype: An int. Dtype of attention_out tensor. Only supprt in PTA graph mode.
...
### AI Core
- input0 query: bfloat16,float8_e4m3fn,float16,hifloat8,int8
- input1 key: bfloat16,float4_e2m1,float8_e4m3fn,float16,hifloat8,int4,int8
- input2 value: bfloat16,float4_e2m1,float8_e4m3fn,float16,hifloat8,int4,int8
- input3 pse_shift: bfloat16,float16,float32
- input4 atten_mask: bool,float16,int8,uint8
- input5 actual_seq_lengths: int64
- input6 actual_seq_lengths_kv: int64
- input7 dequant_scale1: float32,uint64
- input8 quant_scale1: float32
- input9 dequant_scale2: float32,uint64
- input10 quant_scale2: bfloat16,float16,float32
- input11 quant_offset2: bfloat16,float16,float32
- input12 antiquant_scale: bfloat16,float8_e8m0,float16,float32
- input13 antiquant_offset: bfloat16,float8_e8m0,float16,float32
- input14 block_table: int32
- input15 query_padding_size: int64
- input16 kv_padding_size: int64
- input17 key_antiquant_scale: bfloat16,float8_e8m0,float16,float32
- input18 key_antiquant_offset: bfloat16,float8_e8m0,float16,float32
- input19 value_antiquant_scale: bfloat16,float8_e8m0,float16,float32
- input20 value_antiquant_offset: bfloat16,float8_e8m0,float16,float32
- input21 key_shared_prefix: bfloat16,float4_e2m1,float8_e4m3fn,float16,float32,hifloat8,int4,int8
- input22 value_shared_prefix: bfloat16,float4_e2m1,float8_e4m3fn,float16,float32,hifloat8,int4,int8
- input23 actual_shared_prefix_len: int64
- input24 query_rope: bfloat16,float16
- input25 key_rope: bfloat16,float4_e2m1,float8_e4m3fn,float16,hifloat8,int4,int8
- input26 key_rope_antiquant_scale: bfloat16,float8_e8m0,float16,float32,int8
- input27 dequant_scale_query: float8_e8m0,float16,float32
- input28 learnable_sink: bfloat16,float16
- input29 q_start_idx: int64
- input30 kv_start_idx: int64
- output0 attention_out: bfloat16,float8_e4m3fn,float16,hifloat8,int8
- output1 softmax_lse: float32

## Attention Constraints

- Ensure CANN and PyTorch package version compatibility when using this interface with PyTorch.
- Handle empty input: Check if 'query' is empty; return if so.
...
- The shapes of tensors corresponding to 'key' and 'value' must be identical;
