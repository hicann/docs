...
The fusion operator of QuantBatchMatmul, Gelu and mxQuant.

## Inputs

- x1: A matrix tensor. Must be one of the following types: float8_e5m2, float8_e4m3fn
...
batch is optional. 
- x2: A matrix tensor. Must be float8_e4m3fn.
...
ceil(k / x2_k0) == x2_k1. 
- bias: An optional matrix tensor. Must be float32, supports ND format.
The shape is 1D (t,) or 3D (batch, 1, n),
with t equal to n, where n is the same as that of x2.
- x1_scale: An optional matrix tensor. The type supports float8_e8m0, supports ND format.
...
- y: A matrix tensor. The data type is float8_e5m2, float8_e4m3fn. The format supports ND.
...
- transpose_x1: A bool. If true, changes the shape of "x1" from [m, k] to [k, m] before multiplication.
Default: false. Only supports false now.
- transpose_x2: A bool. If true, changes the shape of "x2" from [k, n] to [n, k] before multiplication.
Default: false. Only supports false now.
...
- activation_type: A optional string. The gelu approximation algorithm to use: 'gelu_tanh' or 'gelu_erf', default is
'gelu_tanh'.
- y_dtype: A Int. Declare the output dtype.
- quant_dtype: An optional string. Declare the quant mode.Support mx, pertoken, pertensor and perchannel. Defaults
to "rint".
...
### AI Core
- input0 x1: float8_e4m3fn,float8_e5m2
- input1 x2: float8_e4m3fn
- input2 bias: float32
- input3 x1_scale: float8_e8m0
- input4 x2_scale: float8_e8m0
...
- Inputs does not support tensor with dimension size 0.
- When x2 is ND format, x1 should be ND format.
- In mx quantification, when x1 type and x2 type are both float8_e4m3fn/float8_e5m2 and x1_scale/x2_scale types are
float8_e8m0:
...
- Only weight supports ND and NZ format on Ascend 950 AI Processor. All other inputs and outputs only support ND
format.
