...
Fusion op of matmul and reduce scatter.
...
- x2: A matrix Tensor. The type support float16, bfloat16. The x2 only supports 2 dimensions in current version, for example (K, N). The x2 supports transposed and non-transposed.
...
- reduce_op: A string. A required string identifying the reduction operation to perform. Default: "sum".
