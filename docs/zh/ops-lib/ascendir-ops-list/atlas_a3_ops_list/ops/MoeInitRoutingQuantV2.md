...
compute init routing quant for moe input.
...
- scale: An Optional Tensor. represents the parameters used to compute the result of the quant, which is required
...
- offset: An Optional Tensor. Indicates the offset value used to compute the result, it`s only used in the static
            quant scenario, which is required to be a 1D shape (1,). Type is:Float32. Format support ND.

## Outputs

- expanded_x: A 2D or 3D Tensor. Type is:Int8. Format support ND.
                dropless scenario:
...
- dynamic_quant_scale: A 1D Tensor. represents tthe outputs the intermediate value of the dynamic quant
...
- quant_mode: Optional parameter. Type is:Int32. The value is 0(static quant) or 1(dynamic quant). Default: 0.
