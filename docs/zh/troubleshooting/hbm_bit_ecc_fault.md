...
Device的event日志（slog/dev-os-_id_/run/event/event\_\*.log）中存在“**event\_id=0x80E01809**”或“**event\_id=0x80E01801**”关键字。
...
| 故障影响 | 1. 启动过程中访问到错误地址，可能会启动失败。<br>2. 业务访问到错误地址会返回错误数据，可能导致业务失败。<br>3. 业务未访问到错误地址，不会影响当前业务。 |
| 故障自处理模式 | 1.上报通知事件到设备管理，记录错误地址，尝试在线隔离，重启后进行离线隔离。<br>2.记录错误日志。 |
| 系统处理建议 | 无需操作。 |
...
| 故障自处理模式 | 1.上报故障事件到设备管理。<br>2.记录错误日志。 |
